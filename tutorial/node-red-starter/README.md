...
- パソコン
   - 【推奨】Windows PC
      - パソコンは、webブラウザ上でのNode-RED操作用とゲーム動作マシンの２つの役割に使います。プログラミングする場合は、さらに好みのプログラミング環境をご準備ください。
      - macでもなんとかなります。しかしmacは仕様上ゲームパッドの扱いが難しいです。そこでだいたいのPCゲームはキーボードでも入力を受け付けるので、ゲームパッドをキーボード入力に変換するソフトウェア https://yukkurigames.com/enjoyable/ を用いるか、ゲーム動作マシンとしてmac以外の別のゲーム機を用意してください。       
- ハードウェア
...
- ゲームとして、以下のどれかを用意してください。
    ...
    - あるいは、軽く動作チェックをしたいだけなら、ブラウザでゲームパッドの動作確認ができる https://html5gamepad.com/ をご活用ください。少し表示が専門的かつ、動作が怪しいところがあるのでご注意ください。
...
## 反応速度を上げたい
無線ネットワーク越しにGameControllerizerを制御すると、どうしても遅延が生まれます。だいたい10~50msくらいでしょうか。微々たる時間ですが、ゲームをしている人間にはこの遅延は知覚できます。遅延があってもそれが「もっさりしてるな」という感覚にとどまり、せいぜいゲーム操作の難易度が多少上がるだけであれば、それは慣れで克服できたり、新しい楽しさにつながる可能性を持つものですが、たとえば音楽ゲームなどのように、遅延がゲームの喜びに本質的に悪影響を与える場合は、対策が必要です。
...
にFireFoxブラウザからアクセスしてください。すると、そのPCに接続されているUSBゲームパッドの入力をそのままRaspberry Piに伝送することができます。通常のゲームコントローラ操作に加えてなにか補助的な入力システムを作りたいときなどにご活用ください。また、GcScannerJsはWebSocketを用いてNode-REDと通信するサンプルプログラムでもありますので、興味のある方はソースコードも御覧ください。
